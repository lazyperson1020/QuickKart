...
export default function OrderDetailsScreen() {
  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      ...
      Alert.alert('PDF Generation Failed', e?.message ?? 'Could not generate the invoice. Please try again.');
      return;
    }
    try {
      await FileViewer.open(filePath, { showOpenWithDialog: true });
    } catch (e: any) {
      Alert.alert('Could Not Open File', e?.message ?? 'PDF was saved but could not be opened. Try a PDF viewer app.');
    } finally {
      setDownloading(false);
    }
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F4F5F7' }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        {(() => {
          ...
          const label = isDelivered ? 'Order Delivered' : isCancelled ? 'Order Cancelled' : 'Order Placed';
          const subtitle = isDelivered
            ? `Your items were delivered on ${order.dateString}`
            : isCancelled
            ? `Order was cancelled on ${order.dateString}`
            : `Order placed on ${order.dateString}. Delivery in progress.`;
          return (
        ...
        <View style={{ backgroundColor: '#fff', padding: 16, marginBottom: 12 }}>
          ...
          >
            {downloading
              ...
              : <Text style={{ color: '#7E57C2', fontWeight: '700', fontSize: 13 }}>Download Invoice / Credit Note</Text>
            }
          </TouchableOpacity>
        </View>

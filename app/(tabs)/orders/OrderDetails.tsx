...
export default function OrderDetailsScreen() {
  ...
  const fetchSingleOrderData = async () => {
    ...
    try {
      const orderDocRef = doc(db, 'users', auth.currentUser.uid, 'previousOrders', orderId as string);
  ...
  const formattedDate = t.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) + ', ' + t.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }).toLowerCase();
  
  setOrder({ 
    id: snapshot.id, 
    ...data, 
    items: cleanItemsArray, 
    dateString: formattedDate 
  });
      } else {
      }
    } catch (error) {
      console.error("Failed to execute autonomous order detail matrix fetch:", error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F4F5F7' }}>
      ...
      <View style={{ height: 56, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#E5E7EB', justifyContent: 'space-between' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <TouchableOpacity onPress={() => router.back()} style={{ marginRight: 12 }}><Ionicons name="chevron-back" size={24} color="#111" /></TouchableOpacity>
          <View>
